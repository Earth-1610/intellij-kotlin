...
interface RuleComputer {
    ...
    fun <T : Any> computer(ruleKey: RuleKey<T>, element: PsiElement): T? {
        return computer(ruleKey, element, element) {}
    }

    fun <T : Any> computer(ruleKey: RuleKey<T>, target: Any, context: PsiElement?): T? {
        return computer(ruleKey, target, context) {}
    }

    fun <T : Any> computer(ruleKey: RuleKey<T>, element: PsiElement, contextHandle: (RuleContext) -> Unit): T? {
        return computer(ruleKey, element, element, contextHandle)
    }

    fun <T : Any> computer(ruleKey: RuleKey<T>, target: Any, context: PsiElement?, contextHandle: (RuleContext) -> Unit): T?
}

fun <T : Any> RuleComputer.computer(ruleKey: RuleKey<T>, explicitElement: ExplicitElement<*>): T? {
